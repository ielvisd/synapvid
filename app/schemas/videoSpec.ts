...
export const VisualEventSchema = z.object({
  ...
  params: z.record(z.string(), z.any()).optional().describe('Action-specific parameters'),
...
export const VideoSpecSchema = z.object({
  ...
  audioSegments: z.record(z.string(), AudioSegmentSchema).optional().describe('Audio segments (populated in Stage 2)')
}).refine(
  (spec) => {
    ...
    for (let i = 0; i < sortedScenes.length - 1; i++) {
      ...
      if (currentScene && nextScene && currentScene.end > nextScene.start) {
        return false;
      }
    }
    return true;
